...
const readFile = (req: MedusaRequest, saveLocally?: boolean): Promise<{ fields: formidable.Fields; files: formidable.Files }> => {
    ...
    if (saveLocally) {
        ...
        options.uploadDir = path.resolve("C://Users//Roshini//Downloads//fashion-design-backend//uploads");

        // Define how uploaded files are named.
        options.filename = (name, ext, part, form) => {
            return `${Date.now()}_${part.originalFilename}`; // Unique filename based on the timestamp.
        };
    }
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    const mediaDir = path.resolve("C://Users//Roshini//Downloads//fashion-design-backend//uploads");
    ...
    try {
        ...
        for (const field of uploadedFields) {
            const images = Array.isArray(files[field]) ? files[field] : [files[field]]; // Handle single or multiple files
